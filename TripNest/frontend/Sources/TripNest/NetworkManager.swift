...
class NetworkManager {
    ...
    func addContribution(goalId: Int, amount: Double, completion: @escaping (Contribution?) -> Void) {
        guard let url = URL(string: "\(baseURL)/contributions/?goal_id=\(goalId)") else {
            completion(nil)
            return
        }
        ...
        createAuthenticatedRequest(url: url, method: "POST", body: encodedBody) { request in
            ...
            URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                let decoder = JSONDecoder()
                decoder.dateDecodingStrategy = .iso8601
                let contribution = try? decoder.decode(Contribution.self, from: data)
                completion(contribution)
            }.resume()
        }
    }

    func addMemberToGroup(groupId: Int, email: String, completion: @escaping (Group?) -> Void) {
        guard let url = URL(string: "\(baseURL)/groups/\(groupId)/members?email=\(email)") else {
            completion(nil)
            return
        }

        createAuthenticatedRequest(url: url, method: "POST") { request in
            guard let request = request else {
                completion(nil)
                return
            }
    ...
    func getMessages(groupId: Int, completion: @escaping ([Message]?) -> Void) {
        guard let url = URL(string: "\(baseURL)/groups/\(groupId)/messages/") else {
            completion(nil)
            return
        }
    ...
    func createMessage(groupId: Int, text: String, completion: @escaping (Message?) -> Void) {
        guard let url = URL(string: "\(baseURL)/groups/\(groupId)/messages/") else {
            completion(nil)
            return
        }
